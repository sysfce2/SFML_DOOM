...
//
spriteframe_t R_InstallSpriteLump( int lump, unsigned frame, unsigned rotation, bool flipped )
{
    ...
    {
        logger::error( "R_InitSprites: Sprite %c : %c has two lumps mapped to it", 'A' + frame, '1' + rotation );
    }
...
void R_InitSpriteDefs( const std::array<const std::string, NUMSPRITES> &namelist )
{
    ...
    for ( auto i = 0; i < namelist.size(); ++i )
    {
        ...
        for ( auto l = start + 1; l < end; ++l )
        {
            ...
            auto short_name = wad::lumpinfo[l].name | std::views::take( 4 );
            if ( std::ranges::equal( short_name, spritename | std::views::take( 4 ) ) )
            {
                const auto frame = wad::lumpinfo[l].name[4] - 'A';
                const auto rotation = wad::lumpinfo[l].name[5] - '0';

                if ( modifiedgame )
                    patched = wad::index_of( std::string(wad::lumpinfo[l].name.data(),8) );
                else
                    patched = l;
                ...
                if ( wad::lumpinfo[l].name[6] )
                {
                    const auto frame = wad::lumpinfo[l].name[6] - 'A';
                    const auto rotation = wad::lumpinfo[l].name[7] - '0';
                    if ( frame >= sprites[i].size() )
                    {
                        sprites[i].resize( frame + 1 );
                    }
...
void R_DrawVisSprite( vissprite_t *vis, int x1, int x2 )
{
    ...
    patch = static_cast<patch_t *>( wad::get( vis->patch + firstspritelump ) );
...
#endif
        column = (column_t *)( (std::byte *)patch + patch->columnofs[texturecolumn] );
...
void R_ProjectSprite( mobj_t *thing )
{
    ...
    if ( ( thing->frame & FF_FRAMEMASK ) >= sprdef->size() )
        logger::error( "R_ProjectSprite: invalid sprite frame {} : {} ", static_cast<int>( thing->sprite ), thing->frame );
#endif
...
void R_DrawPSprite( pspdef_t *psp )
{
    ...
    if ( ( psp->state->frame & FF_FRAMEMASK ) >= sprdef->size() )
        logger::error( "R_ProjectSprite: invalid sprite frame {} : {} ", static_cast<int>( psp->state->sprite ), psp->state->frame );
#endif
