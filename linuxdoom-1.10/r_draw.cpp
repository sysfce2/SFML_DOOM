...
void R_DrawColumn(void) {
  ...
  if ((unsigned)dc_x >= SCREENWIDTH || dc_yl < 0 || dc_yh >= SCREENHEIGHT)
    logger::error("R_DrawColumn: {} to {} at {}", dc_yl, dc_yh, dc_x);
#endif
...
#ifdef RANGECHECK
  if ((unsigned)dc_x >= SCREENWIDTH || dc_yl < 0 || dc_yh >= SCREENHEIGHT) {
    logger::error("R_DrawFuzzColumn: {} to {} at {}", dc_yl, dc_yh, dc_x);
  }
#endif
...
void R_DrawSpan(void) {
  ...
  if (ds_x2 < ds_x1 || ds_x1 < 0 || ds_x2 >= SCREENWIDTH ||
    ...
    logger::error("R_DrawSpan: {} to {} at {}", ds_x1, ds_x2, ds_y);
  }
//	dscount++;
#endif
...
void R_InitBuffer(int width, int height) {
  ...
  for (i = 0; i < height; i++)
    ylookup[i] = screens[0].data() + (i + viewwindowy) * SCREENWIDTH;
}
...
void R_VideoErase(unsigned ofs, int count) {
  ...
  //  at one point.
  memcpy(screens[0].data() + ofs, screens[1].data() + ofs, count);
}
